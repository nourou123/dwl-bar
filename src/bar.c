...
void bar_title_render(struct Pipeline *pipeline, struct Bar *bar, cairo_t *painter, int *x, int *y) {
    ...
    // Reserve space for the title, but don't render anything
    ...
    // Set the color of the reserved space to the specified grey color with some transparency
    cairo_set_source_rgba(painter, grey1[0] / 255.0, grey1[1] / 255.0, grey1[2] / 255.0, grey1[3] / 255.0);

    // Fill the reserved space with the specified grey color
    cairo_rectangle(painter, *x, *y, bar->title->width, bar->title->height);
    cairo_fill(painter);

    // Move the x position to the end of the reserved space
    *x += bar->title->width;
}
